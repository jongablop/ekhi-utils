"""
Step 15 — JSON to FER (Sorted)
================================
Pipeline: 2_coatings (Touloukian Vol. 9)

Purpose:
    Exports the final enriched JSON into individual FER-format JSON
    files using ferpy. Orders experiments, extracts temperature ranges
    for filenames, and embeds the Touloukian Vol. 9 citation (including
    Hernicz as co-author) in every source description.

Input:
    - processed_data/all_data_headers_cleaned_corrected_with_measurand.json
      — fully annotated JSON from Step 14
    - properties_symbols_units.csv — symbol/unit lookup from Step 13
    - processed_data/Touloukian-coatings_bibliografia-updated-filtered.csv
      — DOI reference list from Step 03

Output:
    - processed_data/fer_json_sorted/*.json — one FER file per curve,
      named ref_<N>_exp_<E>_<O>_table_<T>_curve_<C>-<Tmin>_<Tmax>.json
        ...
        for entry in entries:
            ...
            for key, value in entry.items():
                ...
                if key not in ("Curve No.", "Ref. No.", "Year", "data", "measurand"):
                    ...
                    # Track experiment ordering: "Above" continues the group
                    ...
                    # Parse temperature range (e.g. "300-1200") for filename
            ...
            # Fall back to cached headers if current table has none (OCR gaps)
            if quantities == []:
            ...
            with open('./processed_data/Touloukian-coatings_bibliografia-updated-filtered.csv', newline='', encoding='utf-8') as csvfile:
            ...
            description = f"Data originally published at {doi}." \
                f" Tabulated data extracted from Y. S. Touloukian, D. P. DeWitt and R. S. Hernicz, *Thermal Radiative Properties: Coatings*, Thermophysical Properties of Matter - The TPRC Data Series, Volume 9, IFI/Plenum, New York, 1972;" \
