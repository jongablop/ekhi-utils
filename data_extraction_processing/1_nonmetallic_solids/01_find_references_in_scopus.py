...
Purpose:
    Searches Scopus by title for each reference in the Touloukian
    bibliography, retrieves DOI, abstract, and BibTeX, and writes
    an augmented Markdown table.

Input:
    - raw_data/Touloukian-ceramicos_refs_only.md — Markdown table of references

Output:
    - raw_data/Touloukian-ceramicos_refs_only_updated.md — table with DOI,
      abstract, and BibTeX columns added

Dependencies:
    - re, pybliometrics

Note:
    Requires a valid Scopus API key configured in pybliometrics.
"""
import re
...
def generate_bibtex(result, abstract=""):
    ...
    if doi:
        bibtex += f"  doi = {{{doi}}},"
    if abstract:
        # Escape curly braces in abstract text for BibTeX compatibility
...
def process_md_file(input_path, output_path):
    ...
    for ref_no, tprc_no, citation in rows[1:]:
        ...
        # Extract the title enclosed in double quotes from the citation string
...
if __name__ == "__main__":
    ...
    pybliometrics.init()
    input_md = "raw_data/Touloukian-ceramicos_refs_only.md"     # Your input Markdown file
    output_md = "raw_data/Touloukian-ceramicos_refs_only_updated.md"  # Output file
